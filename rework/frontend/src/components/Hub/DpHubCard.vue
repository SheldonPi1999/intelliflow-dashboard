<template>
    <v-card class="hub">
        <v-card-title class="hub__title">
            ...
            <v-spacer />
            <!--
            <v-btn class="hub--bg-grey" rounded text>
        ...
        <v-card-text class="hub__container">
            <v-row>
                <div v-for="sensor in hub.sensors" :key="sensor.name">
                    <v-col cols="12">
                        <dp-sensor :name="sensor.id" :type="sensor.type" />
                    </v-col>
                </div>
            </v-row>
        </v-card-text>
        ...
        <v-card-actions class="pa-0">
            <v-container>
                <v-row>
                    <v-col cols="6">
                        {{hub.sensors.length}} / 6 sensors online
                    </v-col>
                </v-row>
            </v-container>
            <!--
            ...
            -->
            <v-btn
                @click="$refs.cliDialog.show()"
                rounded
                text
                width="24px"
                class="px-0 mx-1 hub--bg-grey"
            >
                <v-img max-width="24px" src="@/assets/icons/terminal.svg" />
            </v-btn>
            ...
            <DpCliDialog ref="cliDialog" />
        </v-card-actions>
    </v-card>
</template>
<script lang="ts">
import { Component, Prop, Vue } from 'vue-property-decorator';
import DpCliDialog from '../Dialogs/DpCliDialog.vue';
