    ...
    <v-dialog v-model="dialog.model" persistent max-width="500">
        <v-card min-height="100">
            ...
            <v-card-actions>
                ...
                <v-btn :disabled="!admin" color="green darken-1" text @click="deleteUser(user)">
                    Delete
                </v-btn>
            </v-card-actions>
        </v-card>
    ...
    methods: {
        showDeleteDialog(user: IUser) {
            ...
            if (this.admin === true) {
                ...
                this.dialog.text =
                    'You are not authorized to delete a user. Please contact IT support.';
            }
        ...
        async deleteUser() {
            ...
            await this.$store.dispatch('users/remove', this.user._id);
